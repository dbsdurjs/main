...
# -*- coding: utf-8 -*-
"""
LLM 2 (Qwen2.5-3B-Instruct) 답변 생성
- 국방 도메인 대표 질문에 대한 답변 생성
"""
...
class LLM2ResponseGenerator:
    """LLM 2 (Qwen2.5-3B-Instruct) 답변 생성기"""
    ...
    def load_model(self):
        """모델 로드"""
        print("=" * 80)
        print(f"LLM 2 모델 로드: {self.model_name}")
        print("=" * 80)
    ...
    def generate_response(self, question, max_new_tokens=512, temperature=0.7):
        ...
        # Qwen2.5 Chat 형식 프롬프트
        messages = [
            {"role": "system", "content": "당신은 국방 분야 전문가입니다. 질문에 대해 상세하고 정확하게 답변해주세요."},
    ...
    def generate_all_responses(self, defense_df, max_new_tokens=512, temperature=0.7):
        """모든 질문에 대한 답변 생성"""
        print("\n" + "=" * 80)
        print(f"LLM 2 답변 생성 시작 (총 {len(defense_df)}개)")
        print("=" * 80)
    ...
    def save_results(self, results, format='both'):
        ...
        if format in ['csv', 'both']:
            csv_path = self.output_dir / f'llm2_responses_{timestamp}.csv'
        ...
        if format in ['json', 'both']:
            json_path = self.output_dir / f'llm2_responses_{timestamp}.json'
        ...
        comparator_path = self.output_dir / f'llm2_comparator_{timestamp}.json'
    ...
    def convert_to_comparator_format(self, results):
        ...
        comparator_data = {
            "metadata": {
                ...
                "model_full_name": "Qwen/Qwen2.5-3B-Instruct",
...
def main():
    ...
    # LLM 2 답변 생성기 초기화
    generator = LLM2ResponseGenerator()
    ...
    print(f"\n다음 단계:")
    print(f"  1. LLM Comparator로 두 모델 비교")
